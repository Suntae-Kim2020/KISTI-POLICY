#!/usr/bin/env python3
"""
IBS 유발논문 스캐너 — WoS 원시 TXT에서 FU/FX 필드에 IBS 키워드가 있는 논문을 추출한다.
출력: /Users/kimsuntae/KISTEP/ibs_induced_papers.json (kisti_induced_papers.json과 동일 포맷)
1회성 실행 (~3-5분)
"""
import json
import os
...
OUT = Path("/Users/kimsuntae/KISTEP/ibs_induced_papers.json")

# IBS 키워드 (대소문자 무관)
IBS_PATTERNS = [
    re.compile(r'\bIBS\b', re.IGNORECASE),
    re.compile(r'Institute\s+for\s+Basic\s+Science', re.IGNORECASE),
]
...
def extract_ibs_keywords(text):
    """텍스트에서 매칭된 IBS 키워드 목록을 반환한다."""
    matched = []
    for pat in IBS_PATTERNS:
...
def parse_tab_delimited(filepath, db):
    """탭 구분 WoS TXT 파일을 파싱하여 IBS 유발논문을 추출한다."""
    results = []
    try:
        with open(filepath, "r", encoding="utf-8-sig") as f:
            ...
            for line in f:
                ...
                keywords = extract_ibs_keywords(combined)
                if not keywords:
                    continue
