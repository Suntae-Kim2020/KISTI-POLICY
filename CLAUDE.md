...
├── data_cache.json              # compute.py 실행 결과 (~32MB, 생성 파일)
...
└── templates/
    └── dashboard.html           # 싱글 페이지 대시보드 (Chart.js, 다크 테마, 32개 페이지)
```
...
├── kbsi_induced_papers.json    # KBSI 유발논문 UT 목록
...
### 1단계: 데이터 로딩
- KISTEP 루트의 pickle 심볼릭 링크 6개 + 유발논문 JSON 2개 로딩 (실제 파일은 `generated/`)
...
### 개요
- 요약 대시보드: 4개 카드 + 연도별 합산 차트 + 비교 차트 + 논문 관리
...
| 항목 | KISTI | KBSI |
|------|-------|------|
| 직접 논문 (사사 제외) | 1,956건 | 4,487건 |
| 유발논문 (사사표기 기반) | 6,104건 | 7,810건 |
| 중복(소속+사사) → 유발로 이동 | 624건 | 2,345건 |
| 평균 피인용 (직접) | 53.0회 | 25.0회 |
| 평균 피인용 (유발) | 30.9회 | 22.9회 |
| 총현원 (2021) | 512명 | 392명 |
| 박사인력 (2021) | 257명 | 181명 |
...
7. **C1 필드 국가명 파싱**: WoS C1 필드에서 국가명 추출 시 `", "` 분리 후 마지막 토큰 사용. 대형 공동연구(CDF, CMS 등)에서 저자 이니셜(예: `"Hou, S."` → `"S"`)이 국가로 오인되는 버그가 있었음. `len(country) >= 4` 필터로 해결 (WoS 최단 국가명: IRAN, PERU, CUBA, OMAN = 4자). sec1_4, sec2_6, sec4_4, sec5_6 4곳에 적용.
