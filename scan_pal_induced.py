#!/usr/bin/env python3
"""
PAL(포항가속기연구소) 유발논문 스캐너 — WoS 원시 TXT에서 FU/FX 필드에 PAL 키워드가 있는 논문을 추출한다.
출력: /Users/kimsuntae/KISTEP/pal_induced_papers.json (kisti_induced_papers.json과 동일 포맷)
1회성 실행 (~3-5분)
"""
import json
import os
...
BASE = Path("/Users/kimsuntae/KISTEP/rawdata/wos")
OUT = Path("/Users/kimsuntae/KISTEP/pal_induced_papers.json")

# PAL 키워드 (대소문자 무관)
PAL_PATTERNS = [
    re.compile(r'Pohang\s+Accelerat', re.IGNORECASE),
    re.compile(r'Pohang\s+Light\s+Source', re.IGNORECASE),
    re.compile(r'PAL.XFEL', re.IGNORECASE),
    re.compile(r'PLS.?II', re.IGNORECASE),
]
...
def find_txt_files():
    ...
    # SCIE: rawdata/wos/scie/WoS-SCIE-YYYY/*.txt
    scie_dir = BASE / "scie"
    if scie_dir.exists():
...
def extract_pal_keywords(text):
    """텍스트에서 매칭된 PAL 키워드 목록을 반환한다."""
    matched = []
...
def parse_tab_delimited(filepath, db):
    """탭 구분 WoS TXT 파일을 파싱하여 PAL 유발논문을 추출한다."""
    results = []
    try:
        with open(filepath, "r", encoding="utf-8-sig") as f:
            ...
            for line in f:
                ...
                keywords = extract_pal_keywords(combined)
                if not keywords:
                    continue
